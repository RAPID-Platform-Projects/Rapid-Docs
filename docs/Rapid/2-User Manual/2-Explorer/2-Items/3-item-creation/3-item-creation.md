...
If you are viewing a table that has subtypes, your system administrator may have created additional buttons to create items. For example, in the Rapid Standard Assets Module, there are several "New X Asset"
 buttons that will each create a different subtype of item on the same table. To learn more about this, visit the following link: TODO
...
For example, if your company owned two vehicles with the same Make, Model, and Year, you could use the clone tool to copy this information and reduce the time spent entering data.
...
1)	If the new item has any lookups that can point to the context, each of those lookups will be populated with the context item; otherwise
2)	A new universal link is created between the new item and context item
